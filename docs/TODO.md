...
- [~] A5. iOS build — **規劃已展開，見 [`docs/IOS_PLAN.md`](IOS_PLAN.md)**（2026-07-20）。
      ...
      **✅ 階段 7 收尾**：README 補 iOS 段與 `API_BASE` 平台預設對照表；
      CI 的 `build-ios` job（macos-latest，customer flavor 不簽名 build）**已在 `origin/main`**
      （`.github/workflows/flutter-ci.yml:31`；2026-07-21 覆核，先前記的「token 缺 workflow scope
      推不上去」已不成立）。
...
- [x] **車輛資訊設定頁**：車種**下拉選單**＋車牌輸入 ✅ 2026-07-17（見上方 O2／O3 段）；
      **聯絡電話 ✅ 2026-07-21**（見下方「司機聯絡電話」段）。
...
- [x] **`DriverController` 狀態** ✅ 2026-07-17：`vehicle`／`hasVehicle`／`vehicleChecked`／
      `vehicleLoadFailed`（見上方 O2／O3 段的三態說明）。

## 📞 司機聯絡電話（2026-07-21，三層都補）

> **抓到的缺口**：乘客端「撥打司機電話」按鈕（O7）早就上線，資料讀 `drivers.phone`；
> 但那個欄位**沒有任何寫入路徑**——註冊不收、沒有更新端點、司機端沒有欄位，
> 只有後端測試碼直接寫 DB。正式流程下號碼恆為空字串，`hasPhone` 永遠 false，
> **撥號按鈕從來沒有真正出現過**。規劃早在「司機端／車輛資訊設定頁」寫過
> 「`drivers.phone` 欄位已存在，只是沒填寫入口」，但一直沒人接。

- [x] **後端**（dispatch `claude/driver-phone-profile`，commit `eb46ce1`，**尚未開 PR**）：
      新開 **`PUT /api/driver/profile`**（driver JWT，只改 phone）＋
      `constants.NormalizePhone`／`IsValidPhone`（8–15 碼、允許開頭 `+`）。
      **刻意不併進 `PUT /driver/vehicle`**——那支的 `UpdateVehicle` 會把
      `vehicle_review_status` 原子地重置為 pending（O5），司機每改一次電話
      就會被審核 gate 踢出接單池。空字串＝清除號碼（司機可收回不公開）。
      驗證格式比照車牌的取捨：不綁「09 開頭」，否則市話與國際號碼會被誤擋。
- [x] **App 端**：`DriverProfile` 模型、`fetchProfile`／`updatePhone`、
      controller 的 `phone`／`hasPhone`／`savePhone`（載入失敗**靜默降級**，
      電話是選填，不能讓它擋住車輛設定流程或掛一條無關的紅字）；
      車輛資訊頁加**獨立的電話區塊＋獨立送出鈕**——兩者走不同端點，UI 若共用一顆
      「儲存」，司機改電話就會連帶重送審。頁面明講「修改電話不會影響車輛審核狀態」。

**驗收**：
- 靜態：`flutter analyze` 無 issue、`flutter test` **182 passed**（原 173，新增 9）；
  後端 `go build ./...` OK、`TestDriverSetPhone` 4 子測試＋`TestNormalizePhone`／
  `TestIsValidPhone` 全綠。
- **反向確認**（兩次，都真的跑過）：讓 `UpdatePhone` 也重置審核 → 「改電話不影響車輛
  審核狀態」FAIL；移除電話長度檢查 → 「太短的號碼在本地就擋下」FAIL；兩者皆已還原。
- **後端 live 全鏈路**（新分支 docker）：存 `0912-345-678` → 回 `0912345678`（正規化生效）；
  存完再查車輛 → **`review_status:"approved"`、`can_accept:true`（審核沒被重置，這是關鍵斷言）**；
  存 `12345` → 400「電話格式錯誤」；存 `""` → 清除成功。
  再走完整叫車鏈路（司機上線→乘客建單→接單），乘客端 `GET /customer/rides/active`
  **首次真的帶出 `"driver_phone":"0912345678"`**——改動前此欄恆為空字串。
- **UI 互動實跑 ✅（2026-07-21，iPhone 17 Pro 模擬器＋後端 docker）**：
  新增 `integration_test/driver_phone_test.dart`——在**真模擬器上跑真 App、打真後端**，
  輸入 `0988-123-456` → 點「儲存聯絡電話」→ 斷言 SnackBar「聯絡電話已儲存」出現 →
  **重新向後端查**（不信 controller 的記憶體狀態）得到正規化後的 `0988123456` →
  車輛仍 `approved`／`can_accept:true`。`All tests passed`。
  外部 curl 交叉驗證：`GET /driver/me` 回 `"phone":"0988123456"`（操作前是空字串）。
- **踩到的坑（已記進 [[ios-simulator-automation]]）**：`cliclick` 的點擊在此環境**靜默失效**
  ——`cliclick m:` 移動游標正常、`osascript` 的 keystroke 也有效，但合成滑鼠點擊完全到不了
  模擬器（該執行檔缺輔助使用授權），Tab／方向鍵焦點導航又只能停在第一個 AppBar icon。
  **出路是 `integration_test`**：點擊由 Flutter 自己派送，不經過系統事件，
  同時還能斷言後端狀態。以後 iOS 的 UI 驗收優先走這條，別再跟座標纏鬥。
...
- [x] **司機聯絡方式** ✅ 2026-07-17 乘客端 UI 完成（`DriverVehicleCard` 的 `tel:` 撥號鈕）；
      **但號碼直到 2026-07-21 補上司機端填寫入口才真的會出現**（見上方「司機聯絡電話」段）。

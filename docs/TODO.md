...
- 單元測試：**52 個測試檔、`flutter test` 398 passed**（2026-07-31 預約司機那批實跑；`flutter analyze` 無 issue）。
...
- **App**：`flutter analyze` 無 issue、`flutter test` **398 passed**；
  新增 `test/scheduled_ride_test.dart`（9 支）與 `test/saved_places_test.dart`（12 支）。

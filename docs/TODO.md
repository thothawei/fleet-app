...
- [ ] **行程中顯示進度**：目前到第幾站、下一站是誰。

> ⛔ **上面兩項被後端擋住（2026-07-21 查證）**：乘客端拿不到 stops。
> 後端 `GET /api/customer/rides/active` 回的 `CustomerRideView` 只在 `Ride` 上加了司機聯絡資訊，
> **沒有 stops**；帶 stops 的是司機端的 `DriverRideView`（`internal/service/ride.go`
> `GetActiveRideByDriver` 有 N6 的註解與組裝，customer 那條沒有）。
> 乘客送得出 stops（建單時），卻看不到自己排的行程進度。
> **要解得先改 dispatch**：`CustomerRideView` 比照 driver 帶 stops（含 `arrived_at`／`skipped_at`），
> App 端再加 `CustomerRide.stops` 與地圖／進度 UI。

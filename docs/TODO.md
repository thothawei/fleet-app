...
- [ ] **停靠點編輯 UI**：叫車表單改成可新增／刪除／排序「乘客 + 上車點 + 下車點」（最多 5 位）。
...
> 需求（使用者 2026-07-16）：乘客端顯示司機的**車種與車牌**；司機端**必須先上傳車種車牌才能接單**。
> **主規格見** [line-fleet-dispatch/docs/TODO.md](../../line-fleet-dispatch/docs/TODO.md)「O. 司機車輛資訊」。

**依賴**：後端 O1–O4（`drivers` 車輛欄位、`GET/PUT /api/driver/vehicle`、派單/接單 gate、
`ride.accepted` 帶車輛資訊）。

### 司機端

- [ ] **車輛資訊設定頁**：車種（選單／文字，待拍板）＋車牌輸入，串 `GET/PUT /api/driver/vehicle`。
      入口放 AppBar（比照「我的收入」）或首次登入引導。
- [ ] **上線／接單前置檢查**：未填車輛資訊時，**上線開關停用**並提示「請先設定車種與車牌」，
      點提示直接進設定頁。
      註：**後端也會擋**（O3），App 端只是提早給回饋，不能只靠 App 擋。
- [ ] **`DriverController` 狀態**：加 `vehicle`／`hasVehicle`；`init()`／`login()` 後載入。

### 乘客端

- [ ] **顯示司機車種車牌**：`ride.accepted` 後的「司機前往上車點」階段，
      sheet 目前只顯示「司機：{name}」＋ETA，要加車種與車牌（醒目、方便對車）。
      `CustomerController` 的 `driverName` 旁加 `driverVehicleType`／`driverPlateNumber`。
- [ ] **完成卡／協尋**：是否也顯示當時車輛（依後端是否落車輛快照，見主規格待拍板 4）。

### App 端待拍板

- 車牌顯示格式：是否要放大／等寬字型方便對照？（乘客在路邊要快速比對）
- 既有司機沒有車輛資訊時的引導流程（強制導頁 vs 提示）。

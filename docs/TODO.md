...
- 單元測試：**57 個測試檔、`flutter test` 444 passed**（2026-08-01 第二十八輪後實測；
...
- **WS 斷線期間發生的事**：B 台若在事件送出的當下沒有連線，這次對帳就不會發生。
  目前靠 `onAppResumed` 的無條件 `refreshActive` 自癒——**但那需要 App 進過背景再回來**，
  一台一直開在前景的裝置不會走到。**條件**：等真的有人回報再處理；
  要修的話是「WS 重連成功後補一次對帳」，動的是 `onConnectionChanged`。
- **司機端沒有這條對帳**：司機的 `_handleWsEvent` 對不認得的單也是丟掉，
  但司機端在待命時**有**輪詢（hero 上線後持續回報位置並對帳），情境不同。**未查證，下輪可看。**

---

## 下次任務

> **🎯 2026-08-01 第二十八輪之後的狀態（開工先看這段）**
>

...
> 4. **統計數字在收尾又補了東西之後，要回頭重數一次**（2026-07-31 踩過：
>    預約司機那批寫完「52 個測試檔」，最後又補了一個測試檔就沒回頭改）。
>    **觸發時機很固定**——「已經寫好驗收段落，之後又補了檔案／測試」的那一刻。
>    核對成本是一行：`ls test/*.dart | wc -l`、
>    `grep -hE '^func Test' internal/service/*_test.go | wc -l`。
...
- [📱 第二十八輪：同一帳號的第二台裝置永遠不知道有進行中的行程](#-2026-08-01-第二十八輪同一帳號的第二台裝置永遠不知道有進行中的行程本批修掉-1-個)
- [🔌 第二十九輪：WS 重連後沒有人對帳——司機繼續開往已取消的行程](#-2026-08-01-第二十九輪ws-重連後沒有人對帳司機繼續開往已取消的行程本批修掉-2-個)
...
- 單元測試：**58 個測試檔、`flutter test` 448 passed**（2026-08-01 第二十九輪後實測；
...
## 🚗 2026-07-30 第二十三輪：App 被收掉再開，行程還在但位置回報整段消失（本批修掉 2 個）

> 「定位健康度」這一族的最後一個角落。原本 TODO 寫的是「前景服務被系統收走時 App 端
> 沒有任何偵測」——**查下去發現那句話要修正**：前景服務被收走而 App 還活著時，
> 串流會停，第二十一輪的定期重評已經把它接住了（第二十二輪實測 12～24 秒降級）。
> 真正沒有網的是**行程被還原、但沒有人把定位回報接回來**。

### 根因一句話

`init()` 會用 `_restoreActiveRide()` 把進行中行程還原（行程卡照樣顯示「導航／已上車／完成」），
但 `_online` 一律從 false 起、定位串流也沒起來——**整趟零位置回報**。
而 hero 只寫「離線／目前不會收到派單」：在載客途中，那句話講的是接不接**新單**，
司機不會從它聯想到「乘客看不到我在動」。

**後果不在司機這一端**：乘客端的司機 marker 定格、ETA 不再更新；
後端的抵達圍籬（靠司機座標自動觸發）不會觸發；F3 里程計價的「軌跡 vs 路線取大者」
缺一整段軌跡。觸發條件很日常：省電模式或廠商清背景把 App 收掉、司機自己從最近工作清單滑掉。

### 修法（兩個）

1. **冷啟動恢復回報**：`init()` 還原到進行中行程時接回定位回報。三個限制條件都寫在碼裡——
   - **只在冷啟動做**：App 還活著時 `_online == false` 一定是司機自己按的，
     回前景也自動上線的話那顆離線鈕就按不掉了（`onAppResumed` 刻意不做）。
   - **只在已經有權限時做**：冷啟動就彈系統權限視窗太侵入（司機可能只是想看歷史）。
     沒權限時留一則可行動的訊息（「需要定位權限才能把位置回報給乘客」，
     **不是**「才能上線」——行程中的司機看到後者會以為只影響接新單）。
   - **沒有行程就不自動上線**：接不接單是司機的決定。
2. **`refreshVehicle()` 不再無條件清錯誤**：它原本成功時就 `_setError(null)`，
   於是排在它前面設好的訊息全被洗掉——上面那句權限提示就是這樣消失的（設了、也通知了，
   下一行清空）。改成**只清自己造成的那則**（`_vehicleLoadFailed` 為真才清）。
   同一種病第二十一輪在位置回報探針上修過一次：**清錯誤要指名清哪一則。**

### 驗收

- `flutter analyze` 無 issue、`flutter test` **383 passed**（377 ＋新 6）。
  **與預約司機那批合併後重測：414 passed、54 個測試檔**（兩批的測試互不干擾）。
- **反向驗證兩半各一次**：拿掉冷啟動恢復 → 2 案 FAIL；讓 `refreshVehicle` 回去無條件清錯誤 → 2 案 FAIL。
- **模擬器實跑，同一台裝置、同一張行程 #37，兩個版本各跑一次**（這是本輪最強的證據）：

| 時刻 | 事件 | 後端收到的位置回報 |
|---|---|---|
| 23:21:29–23:22:22 | 上線＋接單 #37 | 每 8 秒一筆 |
| 23:22:25 | `am force-stop`（模擬系統收掉 App） | 停止 |
| 23:23:15 | **修好的版本**冷啟動 | **23:23:23 起恢復**，畫面 hero「上線中」＋行程卡 |
| 23:24:41 | **修改前的版本**冷啟動（同一張單） | **40 秒零筆**，畫面行程卡完整、hero 卻寫「離線」 |

- **負向對照**：沒有進行中行程時冷啟動 → 後端零位置回報（沒有偷偷幫司機上線）。

### 這一輪學到的兩條

1. **接縫的預設實作會跑進測試環境**：我一開始讓冷啟動直接呼叫權限確認，
   結果 `driver_home_widget_test` 從 4.8 秒變成 **7 分鐘以上**——測試環境沒有 platform channel，
   那支呼叫的 Future **永遠不會完成**，`init()` await 下去就把測試卡到 timeout。
   預設實作現在會先看 `FLUTTER_TEST` 直接回「查不到」。
   **判準**：新增任何「`init()` 期間 await 平台能力」的程式碼，先問它在測試環境會回什麼。
2. **「查不到」與「被拒絕」要分開**：權限探針回三態（`LocationPermission?`），
   `null` 什麼都不做、`denied` 才留訊息。混成 bool 的話，測試環境會冒出一則假的權限錯誤。
...
### 🧪 模擬器實跑（m6_pixel ＋ 本機後端）——抓到三個測試層看不到的問題

`flutter test` 400 多支全綠、後端端到端也走通了，把 App 裝到裝置上仍然抓到三個。
**它們有一個共同形狀：邏輯是對的，但畫面上不成立。**

| # | 問題 | 為什麼測試層看不到 |
|---|---|---|
| 1 | **預約入口加在沒被使用的畫面上** | `app.dart` 用的是 `CustomerMapHomeScreen`（地圖版），我加在 `CustomerHomeScreen`（卡片版）。widget 測試各測各的，沒有人問「使用者實際看到的是哪一個」 |
| 2 | **登入後不載入常用地點與預約** | `init()`（冷啟動）有載，`_authenticate()` 沒有。單元測試一律 `setSessionForTest` 再手動呼叫 load，**永遠走不到登入那條路徑** |
| 3 | **車種顯示成 `pet`** | 斷言只檢查了「有沒有顯示車種」，沒檢查顯示的是不是人看得懂的字 |

第 2 個最值得記：`_authenticate` 的註解本來就寫著「比照 `init()` 還原 session」，
卻只帶了 active 與協尋——**新增 init 載入時，要同時問「登入那條路徑呢」**。

**第 1 個的根因查出來了**：`CustomerHomeScreen`（卡片版）**production 完全不用**，
只有 widget 測試在用它（測共用元件比較好架設）——它是死碼，但測試讓它看起來還活著，
所以加錯地方不會有任何訊號。已在該類別頂端標註「production 用的不是這個」，
並補 `test/customer_home_entries_test.dart` 斷言**地圖版**首頁上有預約入口
（反向驗證：把那顆 FAB 的 tooltip 改掉，該支轉紅）。
沒有整批刪掉卡片版——那是這批範圍外的重構，而那些測試現在確實在守共用元件。

實跑同時確認到位的（截圖為證）：
- 叫車頁快捷列四顆：住家／公司／媽媽家／健身房，**圖示依 `kind`、排序住家→公司→其他**；
  已有地點時右上角是「管理」，沒有時是「設定住家／公司」。
- 首頁預約卡「已預約 07/31（五）02:00」＋起訖點。
- 預約頁三區：即將到來 3 筆（近的在前、帶星期）／車已在路上 2 筆／過往 2 筆；
  失敗那筆紅字顯示後端寫的 `last_error`（「已有進行中的訂單」），取消那筆灰字。
- 「我們會在約定時間前 **15 分鐘**開始為你找車」——這個數字取自後端回應，不是寫死的。
- **跨端閉環**：「車已在路上」裡的訂單 `#39`，正是稍早在真環境實跑轉單產生的那一筆。
...
- ~~**司機端也有一個 `default:`**（接單／行程卡的階段判斷）~~
  ✅ **第二十八輪查完了，是乾淨的**，理由見下一段。

---

## 📱 2026-08-01 第二十八輪：同一帳號的第二台裝置永遠不知道有進行中的行程（本批修掉 1 個）

> 先把上一輪留的尾巴收掉，再開新族。

### 先收尾：司機端的 `default:` 是乾淨的（理由不是「App 有防禦」）

司機端 `ActiveRide.fromBackendJson` 的 `phase` 是
`status == pickedUp ? onTrip : enRouteToPickup`——看起來跟乘客端一樣有「未知碼落到某個分支」的風險。
**但後端擋在前面**：`FindActiveByDriver` 的 SQL 是
`status IN (accepted, pickedUp)` 的**白名單**，所以司機端的 `GET /driver/rides/active`
只可能回這兩種狀態。這是**後端契約保證**，不是 App 的防禦——
所以要記住：**哪天後端把新狀態加進那個白名單，這裡就要跟著改**。

WS 那條的 `default: break;` 是忽略未知事件型別，這是對的（事件型別本來就會增加）。

### 本輪的族：多裝置同一帳號

**根因一句話**：`_handleWsEvent` 對「不認得的行程事件」是
`if (active == null || event.rideId != active.rideId) return;`——直接丟掉；
而輪詢在「沒有進行中訂單」時是**停著的**（`_applyActiveRide` 會 `_stopPolling`）。
兩件事湊起來，第二台裝置就**永遠**不會知道有一趟車正在跑。

**這是現在就會踩到的**，不需要後端改任何東西：

1. 同一個乘客帳號登入兩台裝置（手機＋平板，或舊手機還登著）；
2. B 台開著、停在叫車表單；
3. A 台叫車、司機接單——後端的 WS Hub 依 `(角色, id)` 扇出，**B 台收得到** `ride.accepted`；
4. B 台把它丟掉 → 畫面永遠是叫車表單，按下去只會拿到「已有進行中訂單」。

**司機端早就處理過同一件事**（`DriverController` 的 `rideAccepted`：另一台裝置接走了這張單
→ 重讀 active，註解寫明已對真後端實測）。乘客端從來沒有做過。

### 修法與三條邊界

- 不認得的行程事件 → **去跟後端對一次帳**（`refreshActive(silent: true)`），對完就自己接回輪詢。
- **只在 `activeRide == null` 時對帳**。已經有一張單、事件卻是別的 rideId ＝ 舊單的落隊事件
  （乘客一次只有一張進行中訂單），照舊忽略——**這條是既有測試逼出來的**：
  第一版沒收窄，`customer_cancel_notice_test` 的「別的行程的取消事件不觸發通知」立刻紅了。
- **`driver.location` 不觸發**：每幾秒一則的串流，會把一次對帳變成連續打點。
  真正要跟上的是生命週期事件，而那些一定會跟著來。

### 驗收

- `flutter analyze` 無 issue、`flutter test` **444 passed**（440 ＋新 4）、57 個測試檔。
- **先紅再修**：新測試在修改前就是紅的（`activeCalls` 停在 1，事件被丟掉）。
- **反向驗證三個半邊各一次**：
  - 拿掉對帳 → 主案 FAIL。
  - 拿掉 `driver.location` 排除 → 「不觸發連續打點」那案 FAIL。
  - 拿掉 `active == null` 收窄 → **本檔新案與既有的 `customer_cancel_notice_test` 各紅一案**。
- `flutter build apk --debug --flavor customer` 成功。

### 過程中修正的一個自己的錯誤假設

第一版測試斷言「認得的單不該多打 API」——**跑出來是紅的，而錯的是我不是程式**：
`rideAccepted` 本來就會 `refreshActive(silent: true)` 補齊完整資料。
已把那一案改成守「沒有把已知單的路徑改壞，尤其不能變成兩次補讀」。

### 同族還沒碰的角落

- ~~**WS 斷線期間發生的事**~~ ✅ **第二十九輪修掉**（見下一段）。
- ~~**司機端沒有這條對帳**~~ ✅ **第二十九輪查證了，而且比預期嚴重**——
  **上面那句「司機端待命時有輪詢」是錯的，已更正**：司機端**完全沒有行程輪詢**。

---

## 🔌 2026-08-01 第二十九輪：WS 重連後沒有人對帳——司機繼續開往已取消的行程（本批修掉 2 個）

> 上一輪列了兩個候選，本輪查下去發現**它們是同一個洞的兩半**，而且
> **我上一輪寫的那句「司機端待命時有輪詢」是錯的**（下面已更正）。

### 先更正一個錯誤敘述

第二十八輪的「同族還沒碰的角落」寫著「司機端在待命時**有**輪詢」。
**實查是錯的**：`grep Timer lib/driver/driver_controller.dart` 只有
`_locationHealthTimer`（定位健康度重評），**沒有任何行程輪詢**。
司機端的 `_api.activeRide()` 只在三種時機被呼叫：`init()`、自己動作之後
（接單／上車／完成）、`onAppResumed`。

### 根因一句話

`onConnectionChanged` **只記旗標、notifyListeners**，沒有人去問後端；
而 WS 重連**不補送**斷線期間的事件（第四輪就記過）。
兩件事湊起來，斷線視窗裡發生的每一件事都永遠送不到。

### 為什麼司機端最嚴重

| 端 | 斷線視窗的補救 | 結果 |
|---|---|---|
| 乘客端 | 有進行中訂單時每 15 秒輪詢 | 有訂單時會自己校正；**閒置時輪詢是停著的**（第二十八輪那個情境剛好落在這裡） |
| **司機端** | **沒有任何行程輪詢** | 只剩 `onAppResumed`——而那需要 App **真的進過背景**。開車時 App 全程在前景，**走不到** |

最惡劣的具體後果：斷線視窗裡乘客取消 → `ride.cancelled` 送不到 →
**司機繼續開往一個已經取消的上車點**，而畫面上那張行程卡看起來完全正常。

### 修法（兩端同一招）

`onConnectionChanged` 在**重連**時補一次對帳：司機端 `_restoreActiveRide(silent: true)`、
乘客端 `refreshActive(silent: true)`。

- **只在「重連」時做，第一次連上不做**（新增 `_everConnected` 旗標）：
  冷啟動時 `init()` 才剛問過，再問只是多打一支 API。
- **斷線本身不觸發**（問了也問不到）。
- 兩端都走 `silent`：這不是使用者按出來的，失敗不該冒錯誤橫幅。

### 驗收

- `flutter analyze` 無 issue、`flutter test` **448 passed**（444 ＋新 4）、58 個測試檔。
- **先紅再修**：司機端主案在修改前就是紅的（`activeCalls` 停在 1）。
- **反向驗證三處**：
  - 拿掉司機端的重連對帳 → 主案 FAIL；
  - 拿掉 `_everConnected` 守衛 → 「第一次連上不重複對帳」FAIL（變成 2 次）；
  - 拿掉乘客端的重連對帳 → 乘客端那案 FAIL。
- `flutter build apk --debug --flavor customer` 成功。

### 過程中修正的一個測試錯誤

第一版司機端測試**跳過了「第一次連上」**，於是 `_everConnected` 永遠是 false、
重連判定不成立，測試紅得像是修法沒生效。**錯的是測試不是程式**——
production 一定會先連上一次。已補上 `ws.setConnected(true)` 當作初次連線。
這與上一輪那條是同一個教訓：**測試紅了要先問「是我的假設錯了嗎」**。

### 同族還沒碰的角落

- **`onConnectionChanged` 收不到「連線其實已經死了」的情況**（弱網：連得上但通不了，
  凍結的後端不會送 FIN）。那一族在第六輪處理過一半（司機端有 `locationStale` 偵測），
  但**重連對帳幫不上忙——因為根本沒有「重連」事件**。
  **條件**：要修得靠心跳逾時主動斷線重連，動的是 `FleetWsClient`。
- **乘客端閒置時仍然沒有輪詢**：本輪讓「重連」與「收到不認得的事件」兩條路補上了，
  但「WS 一直連著、事件也真的沒送到」這種情況還是沒人管。
  **條件**：那要嘛信任後端一定會送、要嘛開閒置輪詢（每台裝置每 15 秒一支 API）——
  現在的證據不足以支持後者的成本。
...
>
> 1. **`gh pr list`（三個 repo）照樣是第一件事**——本輪開工時皆為 0。
> 2. **第十二次實查三個外部卡點：依然都不在**。
> 3. **這一輪學到的一條（最重要）**：
>    **上一輪寫進「還沒碰的角落」的敘述，下一輪要當成假設去查，不能當成事實。**
>    第二十八輪我寫「司機端待命時有輪詢，情境不同」——**那句是錯的**，
>    司機端根本沒有行程輪詢，而且因此是兩端裡最嚴重的一邊。
>    寫「未查證」三個字有用，但**只有真的去查才算數**。
> 4. **下一輪候選**（依價值排序）：
>    - **弱網下沒有「重連」事件**：凍結的後端不會送 FIN，`onConnectionChanged`
>      永遠不會 false→true，本輪的對帳幫不上忙。要修得靠心跳逾時主動斷線重連，
>      動的是 `FleetWsClient`。**這是本輪對帳唯一補不到的缺口，最具體。**
>    - **App 版本比後端新**：App 送出後端不認得的欄位／值時，後端是忽略還是 400？
>      三端都沒寫明這個契約。
>    - **登出／換帳號時的殘留**（從沒系統掃過）：第二十四／二十八輪各修過一個
>      「上一位使用者的資料留著」，但沒有人把 `_clearSession` 的欄位與
>      controller 的所有狀態欄位對過一次帳。

>
> 1. **`gh pr list`（三個 repo）照樣是第一件事**——本輪開工時皆為 0。
> 2. **第十一次實查三個外部卡點：依然都不在**。
> 3. **這一輪學到的一條**：**既有測試是防線，紅了要先問「是不是我改太廣」。**
>    本輪第一版把「所有不認得的行程事件」都拿去對帳，
>    `customer_cancel_notice_test` 立刻紅——那條測試守的是
>    「別的行程的取消事件不該影響這一趟」，而我的改動剛好踩過去。
>    收窄成「只在手上什麼都沒有時對帳」之後，兩邊都成立。
>    **另一條**：測試紅了不一定是程式錯——本輪有一案是**我的假設錯**
>    （`rideAccepted` 本來就會補讀），該改的是測試。
> 4. **下一輪候選**（依價值排序）：
>    - **司機端有沒有同一個洞**（本輪的直接延伸）：司機的 `_handleWsEvent` 對不認得的單
>      也是丟掉，但司機端待命時有輪詢，情境不同。**未查證**，這是最便宜的一條。
>    - **WS 重連後補一次對帳**：本輪的洞在「事件送出當下沒連線」時仍然存在，
>      目前只靠 `onAppResumed` 自癒，一直開在前景的裝置走不到。
>    - **App 版本比後端新**：App 送出後端不認得的欄位／值時，後端是忽略還是 400？
>      目前三端都沒寫明這個契約。
...
>    ✅ **已於第二十三輪查清並修掉**，但**原本的描述要更正**：
>    前景服務被收走而 App 還活著時，串流會停，第二十一輪的定期重評已經接住了
>    （第二十二輪實測 12～24 秒降級）。真正沒有網的是**App 被收掉再開**——
>    行程會被還原，但沒有人把定位回報接回來，整趟零回報。詳見第二十三輪專段。
>
> **➡️ 定位健康度這一族到此清完**（司機端串流健康度、乘客端叫車出口、冷啟動恢復回報）。
> 下一輪請換族。讀碼時順手記下、還沒查的候選：
> - **hero 在行程中仍寫「等待派單中」**（第二十三輪實跑畫面看到）：司機已經在載客，
>   那句話講的是接新單，語意不精確。屬文案／狀態顯示，不影響功能。
> - **司機端沒有任何輪詢**：WS 漏事件時只有回前景與操作被 409 打回兩條補救路徑。
>   第四輪修過回前景對帳，但**長時間停留在前景**的漏事件情境沒有網。
>

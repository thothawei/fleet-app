...
> 建立：2026-07-08 盤點（以程式碼實測為準）。最後盤點：**2026-07-29**。
> 編號沿用後端 repo 的
...
- 單元測試：**40 個測試檔、`flutter test` 291 passed**（2026-07-29 實跑；`flutter analyze` 無 issue）。
...
>    做在前面只會做出沒人用的流程。
> 8. **清掉殘留 worktree `fleet-improvements-todo-cc8d85`**（2026-07-29 盤點新增）：
>    分支 `claude/e2e-salvaged-fixes` 的內容**全部在 main**、工作區乾淨，
>    三條判定依據見 [第九輪盤點](#-2026-07-29-第九輪盤點下一輪的三個候選本批只盤點沒有修)的「維護項 8」。
>    刪之前先確認沒有 session 正在用它。
>
...
>
> **🎯 下次開工第一件事**（2026-07-29 更新）：先跑 `gh pr list`，再看
> [第九輪盤點](#-2026-07-29-第九輪盤點下一輪的三個候選本批只盤點沒有修)的三條候選——
> **這一次不是「沒有可做的事」**：候選 2（聊天室補讀）與候選 3（`completeTrip` 逾時對帳）
> 都**不需要任何外部資源**，是目前投報率最高的兩件。
> 維護項只剩 7（等營運需求）與 8（刪殘留 worktree，5 分鐘的事）。
...
> | # | 候選 | 需要外部資源？ | 建議順序 |
> |---|---|---|---|
> | 1 | **乘客端完全沒有推播管道**（後端 `/api/customer/device-token` 已就緒、App 從沒接） | 憑證那半要 Firebase（同 A2）；**接線那半不用** | 先做不需憑證的接線 |
> | 2 | **聊天室的 `afterId` 增量補讀是死路徑**——切背景／WS 斷線期間的訊息不會補回，連未讀角標都不亮 | 不用 | **最划算**：缺的只是一個觸發點 |
> | 3 | **`completeTrip`／`pickUpPassenger`／`_markStop` 沒有逾時對帳**（第六輪只補了接單與建單） | 不用 | 先修 `completeTrip`，另兩條嚴重度低很多 |
>
> ⚠️ **原本列在這裡的「乘客端被系統殺掉後從推播喚醒的還原路徑」已刪除**——
> 查證後發現乘客端根本沒有推播（candidate 1），**那題的前提不成立**。

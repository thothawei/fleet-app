...
## 📱📱 2026-07-28 第六輪 debug：多裝置／跨管道（同一司機的另一處動作）

> 清單上的第 2 項。**先推翻自己的假設**：原本猜「後端可能只保留最後一條連線 →
> 兩台互踢無限循環」。讀 `internal/events/hub.go` 查證後**不成立**——
> Hub 是 `map[*Client]bool`，同一個 driver id 的**每一條連線都會收到事件**，
> 不覆蓋也不互踢。所以多裝置本來就被支援，真正的問題是**事件到了卻沒人處理**。

**兩個缺口**（都不是多裝置獨有，LINE 那條路徑同樣會踩到）：

1. **別處接了單，這台毫無反應**。`ride.accepted` 的收件人就是「接單的那個司機」，
   所以這台收到它＝「我在別的地方接了這張單」。舊碼只在 `_activeRide?.rideId == event.rideId`
   時處理（也就是只認自己這台接的），其他情況什麼都不做：**接單卡繼續留在畫面上、
   行程卡不出現**，司機以為還沒接到，按下去才發現。
2. **別處標記了停靠點，這台停在舊的「下一站」**。乘客端早就會收 `ride.stop_updated`
   並更新進度，司機端卻連這個 case 都沒有。而「下一站」正是司機端唯一給操作的那一站——
   兩台會對不同的站給按鈕。

**修法**：
- App（本批）：`ride.accepted` 不是自己這台接的 → 收掉接單卡＋以後端為準重讀 active
  （事件 payload 不含全程 stops，重讀才會齊全）；新增 `ride.stop_updated` 的處理，
  同一張單就重讀 active。
- **後端（dispatch PR #53，同批）**：`ride.stop_updated` 原本**只推給乘客**——
  所以缺口 2 的 App 端處理在舊後端下是死碼（實跑當場證實：標記後 8 分鐘畫面沒動）。
  後端 `publishStops` 改成乘客與司機各推一則。

**驗收**：
- 靜態：`flutter analyze` 無 issue、`flutter test` **261 passed**（257 ＋新 4）。
  新增 `test/driver_multi_device_test.dart`（別台接走→收卡＋補行程／自己這台照舊不重打 active／
  別處標記→重讀跟上／不是這張單不動）。**反向確認**：兩處修正各自拿掉後 2 案 FAIL。
- **✅ 模擬器實跑（`m6_pixel` driver flavor ＋ docker compose，2 乘客 4 站的 ride #9）**：
  - **別台接單**：App 亮著接單卡（「多乘客行程（4 站）」）→ **用同一司機的 API token 接單**
    → **司機沒按 App 任何按鈕**，接單卡自動消失、行程卡自動出現，且**含完整 4 站清單與多點地圖**。
  - **別處標記停靠點**：舊後端下標記第 1 站 → **8 分鐘後畫面完全沒動**（缺口 2 的實證）；
    換上 dispatch PR #53 的後端後標記第 2 站 → **5 秒內**前兩站變綠勾「已完成」、
    下一站前移到「乘客 A下車」並帶操作鈕、地圖重畫。

**查了但沒動的**（多裝置的其餘行為，屬後端／產品決策）：
- 兩台同時上線時**位置互相覆蓋**（Redis 單一 `driver:<id>:loc`），派單距離會以最後回報的那台為準。
- 後端**沒有踢舊 session 的機制**（JWT 無狀態）。要不要「一個帳號只能一台上線」是產品決策，
  不該由 App 端片面實作。
...
> 2. ~~**多裝置／同帳號重複登入**~~ ✅ **已完成（2026-07-28 第六輪）**，見上方專段
>    （原本猜的「兩台互踢」不成立；真正的問題是事件到了沒人處理，順帶補了後端一則事件）。

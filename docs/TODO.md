...
- 單元測試：**52 個測試檔、`flutter test` 400 passed**（2026-07-31 預約司機那批實跑；`flutter analyze` 無 issue）。
...
- **App**：`flutter analyze` 無 issue、`flutter test` **400 passed**；
  新增 `test/scheduled_ride_test.dart`（11 支）與 `test/saved_places_test.dart`（12 支）。

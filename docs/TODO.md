...
- **未做：模擬器實跑**。要證實的話——
  聊天室：兩端同時開同一張單的聊天室，一端按 HOME（或 `docker pause` 後端）、
  另一端送兩則、再回前景，看訊息是否補上；
  逾時對帳：`docker pause` 凍結後端（**模擬器限速對 `10.0.2.2` 無效**，第六輪的教訓）→
  按「完成行程」→ 等逾時 → 解凍 → 看行程卡是否自己收掉。
  ⚠️ 第六輪誠實記錄過：`docker pause` **造不出**「後端其實成功、只是回應遺失」的正向競態
  （dio 逾時會關掉連線，凍結中的後端根本沒讀到請求）。要在真裝置驗正向分支，
  需要一支「轉發請求但吃掉回應」的代理——這也是這兩項目前只有測試層證據的原因。

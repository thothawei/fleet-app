...
**實跑發現（環境層，非本次程式問題）**：模擬器上 WS 曾 `Connection timed out`（logcat 可見），
導致派單事件收不到、接單卡不跳；重啟 App 讓 WS 重連後恢復。畫面上的「即時連線正常」
是 `_wsConnected` 旗標，**斷線後未即時反映**——值得追一下 WS 斷線的 UI 呈現（待辦）。

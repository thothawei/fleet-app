...
> **💰 高優先（使用者要求 2026-07-15）：金額改用整數台幣（無小數）。** 台幣沒有小數點、支付不能用小數。
> 目前 `lib/core/util/money.dart` 顯示「分→NT$XX.XX（兩位小數）」，司機收入頁、乘客完成卡車資、
> **遺失物處理費與支付金額**都會出現 `NT$21.03`、`NT$17.96` 這種不可支付金額。待後端把車資/手續費/會費/
> 處理費/報表改成整數元後，App 這端同步：`money.dart` 顯示不再有小數、支付金額為整數元。**主規格與決策見**

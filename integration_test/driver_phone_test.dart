...
void main() {
  ...
  testWidgets('司機填聯絡電話 → 真的寫進後端，且車輛審核狀態不受影響', (tester) async {
    ...
    // main 的 GET /driver/vehicle 順帶回 phone，所以一次查詢就能同時驗兩件事
    // （原始 PR 走的是另一支 fetchProfile，main 沒有這條路徑）。
    final fresh = await api.fetchVehicle();
    expect(fresh.phone, '0988123456', reason: '後端應存下正規化後的號碼');

    // 這是本功能存在的理由：改電話不可以把司機踢回審核中。
    expect(fresh.canAccept, isTrue, reason: '存電話後仍必須能接單');
    expect(fresh.reviewStatus.name, 'approved');
  });
}
